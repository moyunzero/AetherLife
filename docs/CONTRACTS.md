...
| **Join** | `joinVicinity` → 8s 窗口内 NPC 朝发起者邻近格移动；该窗口内该 NPC **绕过 soft leash**，并打断 walk/pause hold（D-11）；`maxRadius===0` 钉死仍优先于 join；worker 每 NPC 每 game-day bucket（480 分钟）最多 2 次 |
